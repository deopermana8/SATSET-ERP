...
export default function Page() {
  ...
  financeService.createCashAccount({ code: "1010", name: "Main Cash", openingBalance: 5_000_000 });
  ...
  crmKernel.membershipService.registerTier({ code: "silver", name: "Silver", minimumSpend: 1_500_000, pointMultiplier: 1.2, birthdayBonusPoints: 100 });
  crmKernel.membershipService.registerTier({ code: "gold", name: "Gold", minimumSpend: 4_000_000, pointMultiplier: 1.5, birthdayBonusPoints: 150 });
  crmKernel.membershipService.registerTier({ code: "platinum", name: "Platinum", minimumSpend: 8_000_000, pointMultiplier: 2, birthdayBonusPoints: 250 });

  crmKernel.crmService.registerSegment({ code: "general", name: "General", minimumSpend: 0, minimumVisits: 0 });
  crmKernel.crmService.registerSegment({ code: "family", name: "Family Traveler", minimumSpend: 2_000_000, minimumVisits: 3, membershipTierCode: "silver" });
  crmKernel.crmService.registerSegment({ code: "vip", name: "VIP Guest", minimumSpend: 6_000_000, minimumVisits: 5, membershipTierCode: "gold" });
  ...
  crmKernel.crmService.recordVisit({ customerId: customerA.id, source: "reservation", amount: 1_250_000, packageName: "Family Adventure" });
  crmKernel.crmService.recordVisit({ customerId: customerA.id, source: "cafe", amount: 240_000, cafeMenuName: "Nasi Goreng" });
  crmKernel.crmService.recordVisit({ customerId: customerA.id, source: "souvenir", amount: 180_000, souvenirName: "T-Shirt" });
  crmKernel.crmService.recordVisit({ customerId: customerA.id, source: "reservation", amount: 1_600_000, packageName: "Safari Night" });
  crmKernel.crmService.recordVisit({ customerId: customerB.id, source: "reservation", amount: 750_000, packageName: "Mini Explorer" });
  crmKernel.crmService.recordVisit({ customerId: customerB.id, source: "cafe", amount: 130_000, cafeMenuName: "Kopi Susu" });
  ...
  crmKernel.loyaltyService.rewardPoints({ customerId: customerA.id, amount: 1_250_000, reference: "RES-POINT-001", note: "Reservation reward" });
  crmKernel.loyaltyService.rewardPoints({ customerId: customerA.id, amount: 420_000, reference: "ONSITE-POINT-002", note: "On-site spending reward" });
  ...
  crmKernel.voucherService.generateVoucher({ customerId: customerA.id, value: 100_000, minimumSpend: 500_000, validDays: 30 });
      ...
      <div className="mx-auto flex max-w-6xl flex-col gap-6">
        ...
        <div>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <FinanceDashboard
              totalCashAccounts={financeSnapshot.cashAccounts.length}
              totalBankAccounts={financeSnapshot.bankAccounts.length}
              cashTransactionCount={financeSnapshot.cashTransactions.length}
              bankTransactionCount={financeSnapshot.bankTransactions.length}
              expenseCount={financeSnapshot.expenses.length}
              incomeCount={financeSnapshot.incomes.length}
              transferCount={financeSnapshot.transfers.length}
              pettyCashCount={financeSnapshot.pettyCashAccounts.length}
              latestCashBalance={latestCashBalance}
            />
            <AccountingDashboard
              totalAccounts={accountingSnapshot.chartOfAccounts.length}
              postedJournals={accountingSnapshot.journalEntries.length}
              trialBalanceDebit={accountingSnapshot.trialBalance.totalDebit}
              trialBalanceCredit={accountingSnapshot.trialBalance.totalCredit}
              netIncome={accountingSnapshot.incomeStatement.netIncome}
              totalAssets={accountingSnapshot.balanceSheet.totalAssets}
              totalLiabilities={accountingSnapshot.balanceSheet.totalLiabilities}
              totalEquity={accountingSnapshot.balanceSheet.totalEquity}
            />
          </div>
          <div className="mt-6 space-y-6">
            <JournalViewer
              entries={generalJournal.map((entry) => ({
            ...
            />
            <LedgerViewer
              ledgers={generalLedger.map((ledger) => ({
            ...
            />
            <TrialBalanceView
              totalDebit={accountingSnapshot.trialBalance.totalDebit}
              totalCredit={accountingSnapshot.trialBalance.totalCredit}
            />
            <BalanceSheetView
              totalAssets={accountingSnapshot.balanceSheet.totalAssets}
              totalLiabilities={accountingSnapshot.balanceSheet.totalLiabilities}
            ...
            />
            <IncomeStatementView
              totalRevenue={accountingSnapshot.incomeStatement.totalRevenue}
              totalExpense={accountingSnapshot.incomeStatement.totalExpense}
              netIncome={accountingSnapshot.incomeStatement.netIncome}
            />
            <CashFlowView
              operating={accountingSnapshot.cashFlow.operating}
              investing={accountingSnapshot.cashFlow.investing}
              financing={accountingSnapshot.cashFlow.financing}
              netChangeInCash={accountingSnapshot.cashFlow.netChangeInCash}
            />
            <ClosingWizard
              periods={accountingSnapshot.fiscalPeriods.map((fiscalPeriod) => {
        ...
        <div>
          ...
          <div className="grid gap-6 lg:grid-cols-2">
            <CustomerDashboard
              ...
              totalVisits={crmStats.totalVisits}
              totalSpending={crmStats.totalSpending}
              averageSpending={crmStats.averageSpending}
              activeMemberships={crmStats.activeMemberships}
            />
            <MembershipDashboard
              members={Array.from(crmKernel.store.memberships.values()).map((membership) => ({
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-2">
            <VoucherDashboard
              vouchers={Array.from(crmKernel.store.vouchers.values()).map((voucher) => ({
                code: voucher.code,
            ...
            />
            <PromotionDashboard
              promotions={Array.from(crmKernel.store.promotions.values()).map((item) => ({
                code: item.code,
          ...
          <div className="mt-6 grid gap-6 lg:grid-cols-2">
            <CustomerProfile
              customerCode={customerA.customerCode}
              fullName={customerA.fullName}
              email={customerA.email}
              phone={customerA.phone}
              membershipTier={customerATier?.name ?? "Bronze"}
              points={customerAMembership?.points ?? 0}
              totalSpending={customerAStats.totalSpending}
            />
            <CustomerAnalytics
              totalVisits={customerAStats.totalVisits}
              totalSpending={customerAStats.totalSpending}
              visitFrequencyPerMonth={customerAStats.visitFrequencyPerMonth}
              favoritePackage={customerAStats.favoritePackage}
              favoriteCafeMenu={customerAStats.favoriteCafeMenu}
              favoriteSouvenir={customerAStats.favoriteSouvenir}
            />
          </div>
          <div className="mt-6 grid gap-6 lg:grid-cols-3">
            <CustomerSearch
              query="sa"
              results={customerSearchResults.map((item) => ({
            ...
            />
            <CustomerHistory
              visits={crmKernel.store.visitHistories
            ...
            />
            <PointHistory
              transactions={crmKernel.store.pointTransactions
          ...
          <div className="mt-6 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <ul className="mt-3 space-y-2">
              {customerATimeline.map((item) => (
                <li key={`${item.type}-${item.occurredAt.toISOString()}-${item.description}`} className="rounded-xl border border-slate-200 p-3 text-sm text-slate-700">
