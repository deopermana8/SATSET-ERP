...
export async function PATCH(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (body?.description !== undefined) updateData.description = String(body.description) || null;
    if (body?.destinationId !== undefined) updateData.destinationId = Number(body.destinationId) || undefined;
    if (body?.active !== undefined) updateData.active = Boolean(body.active);

    try {
      const updated = await prisma.facility.update({ where: { id: parsedId }, data: updateData });
...
export async function DELETE(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    await prisma.facility.update({ where: { id: parsedId }, data: { deletedAt: new Date() } });
