...
test('outbound module regression', async ({ page }) => {
  const errors = captureErrors(page);

  await page.goto('/');
  await waitForRuntimeReady(page);
  await clickSidebarItem(page, 'Outbound');
  await closeModalIfOpen(page);

  const state = await snapshotState(page);
  expect(state.moduleTitle).toBe('Outbound');
  expect(state.moduleSubtitle).toBe('Kelola data Outbound');
  ...
  await expectElementScreenshotMatchesBaseline(page, '#content', baselineScreenshotPath('outbound.png'));
