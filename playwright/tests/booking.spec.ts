...
test('booking module regression', async ({ page }) => {
  const errors = captureErrors(page);

  await page.goto('/');
  await waitForRuntimeReady(page);
  await clickSidebarItem(page, 'Booking');
  await closeModalIfOpen(page);

  const state = await snapshotState(page);
  expect(state.moduleTitle).toBe('reservasi');
  expect(state.moduleSubtitle).toBe('Kelola data reservasi');
  ...
  await expectElementScreenshotMatchesBaseline(page, '#content', baselineScreenshotPath('booking.png'));
