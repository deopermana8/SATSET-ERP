...
test('finance module regression', async ({ page }) => {
  const errors = captureErrors(page);

  await page.goto('/');
  await waitForRuntimeReady(page);
  await clickSidebarItem(page, 'Finance');
  await closeModalIfOpen(page);

  const state = await snapshotState(page);
  expect(state.moduleTitle).toBe('Keuangan Inti');
  expect(state.moduleSubtitle).toBe('Kelola data Keuangan Inti');
  ...
  await expectElementScreenshotMatchesBaseline(page, '#content', baselineScreenshotPath('finance.png'));
