...
export class DocumentationAgent implements IEngine {
  public readonly name = "DocumentationAgent";
  ...
  async run(context: Context): Promise<void> {
    ...
    history.push({ agent: this.name, status: "completed", confidence: 0.77 });
    context.metadata = {
      ...context.metadata,
      agentMesh: {
        ...(mesh ?? {}),
