...
import { PipelineResolver } from "../src/runtime/PipelineResolver.js";
...
async function main(): Promise<void> {
  ...
  const resolver = new PipelineResolver(registry);
  const pipeline = resolver.resolve([first, second]);
  assert.ok(pipeline.length >= 2);
  console.log("pipeline resolver test passed");
}
