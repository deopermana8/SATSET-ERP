...
try {
    ...
    $ResolvedName = if ([string]::IsNullOrWhiteSpace($Name)) { 'column' } else { $Name }
    $EntityName = Test-SatsetModuleName -Name $ResolvedName

    Write-Host ""
    Write-Host "[INFO] Starting column generation for '$EntityName'" -ForegroundColor Cyan

    $Blueprint = Get-SatsetBlueprint "column"
    ...
    Write-Host "[ OK ] Column generation completed" -ForegroundColor Green
    exit 0
}
catch {
