...
try {
    ...
    $ResolvedName = if ([string]::IsNullOrWhiteSpace($Name)) { 'route' } else { $Name }
    ...
    $OutputRoot = Join-Path (Get-SatsetRootPath) 'apps\admin\routes'
    $Blueprint = Get-SatsetBlueprint 'route'
    ...
    Write-Host "[ OK ] Route generation completed" -ForegroundColor Green
    exit 0
}
catch {
