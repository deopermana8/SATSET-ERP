...
. "$PSScriptRoot\..\engine\OpenApiEngine.ps1"

$ModuleName = Test-SatsetModuleName -Name $Name

$Metadata = Get-SatsetMetadata -ModuleName $ModuleName

$TemplateValues = Get-SatsetOpenApiTemplateValues `
    -ModuleName $ModuleName `
    -Metadata $Metadata

$Blueprint = Get-SatsetBlueprint "openapi"

$OutputRoot = Join-Path (Get-SatsetRootPath) "docs\openapi"
...
Write-Host ""
Write-Host "[ OK ] OpenAPI generation completed." -ForegroundColor Green
exit 0
