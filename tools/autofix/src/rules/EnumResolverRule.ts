import { BaseRule } from "./BaseRule.js";
import { BuildError, BuildErrorType, PatchPlan } from "../types.js";

export default class EnumResolverRule extends BaseRule {
  constructor() {
    super({
      capabilities: ["update-type"],
      dependencies: [],
      description: "Resolve enum type mismatches.",
      name: "EnumResolverRule",
      priority: 150,
      targets: [BuildErrorType.TYPE_ERROR],
      version: "2.0.0"
    });
  }

  supports(error: BuildError): boolean {
    return /enum/i.test(error.raw);
  }

  async createPatch(error: BuildError): Promise<PatchPlan | null> {
    const targetName = this.inferContainerName(error.message);
    if (!error.file || !targetName) {
      return null;
    }

    return {
      ruleName: this.manifest.name,
      summary: `Update enum backed type in ${targetName}`,
      operations: [
        {
          kind: "update-type",
          file: error.file,
          targetName,
          nextValue: "string"
        }
      ]
    };
  }
}
